        ...
        for (int j = 0; j < width; ++j)
        {
            ...
            for (int k = 0; k < window_size; ++k)
            {
                for (int l = 0; l < window_size; ++l)
                {
                    ...
                    if (i + k - rmax < 0 || i + k - rmax >= height || j + l - rmax < 0 || j + l - rmax >= width)
                        continue;
...
int main(int argc, char **argv)
{
    ...
    cv::Mat low_disp;
    cv::resize(disp, low_disp, cv::Size(), 0.1, 0.1);
    cv::imwrite("lowres_disp.jpg", low_disp);

    cv::Mat upsampled_disp = upsample(sigma_s, sigma_r, low_disp, image);
